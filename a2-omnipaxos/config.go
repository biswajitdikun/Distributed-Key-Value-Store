...
func (cfg *config) apply(server int, m omnipaxoslib.ApplyMsg) {
	cfg.mu.Lock()
	defer cfg.mu.Unlock()
...
func (cfg *config) applier(server int, applyCh chan omnipaxoslib.ApplyMsg, stopCh <-chan struct{}) {
	for {
		select {
		case <-stopCh:
			{
...
func (cfg *config) start1(i int, applier func(int, chan omnipaxoslib.ApplyMsg, <-chan struct{})) {
	cfg.crash1(i)
	...
	applyCh := make(chan omnipaxoslib.ApplyMsg)
	cfg.stopCh[i] = make(chan struct{})
	go applier(i, applyCh, cfg.stopCh[i])

	// rf := Make(ends, i, cfg.saved[i], applyCh)
	endsCopy := make([]omnipaxoslib.Callable, len(ends))
	for j, end := range ends {
		endsCopy[j] = end
	}
	rf := Make(endsCopy, i, cfg.saved[i], applyCh)
